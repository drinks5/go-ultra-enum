...
func (x *{{.NewName | LcFirst}}Enum)UnMarshalName(name string) (*{{.NewName | LcFirst}}Ele, error){
	if val, ok := {{.NewName | LcFirst}}EnumNameMap[name]; ok{
		return val, nil
	}
	return nil, fmt.Errorf("%s does not belong to names", name)
}
func (x *{{.NewName | LcFirst}}Enum)UnMarshalValue(value {{.Tpe}}) (*{{.NewName | LcFirst}}Ele, error){
...
func (x *{{.NewName | LcFirst}}Ele) MarshalJson() ([]byte, error) {
